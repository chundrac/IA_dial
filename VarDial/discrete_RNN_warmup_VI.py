...
def gen_var_dist():
    loc_q_U,scale_q_U = tf.get_variable('loc_q_U',[L,K]),tf.nn.softplus(tf.get_variable('scale_q_U',[L,K]))+1e-35
    loc_q_W_x,scale_q_W_x = tf.get_variable('loc_q_W_x',[X,K,J]),tf.nn.softplus(tf.get_variable('scale_q_W_x',[X,K,J]))+1e-35
    loc_q_W_h,scale_q_W_h = tf.get_variable('loc_q_W_h',[K,J,J]),tf.nn.softplus(tf.get_variable('scale_q_W_h',[K,J,J]))+1e-35
    loc_q_W_y,scale_q_W_y = tf.get_variable('loc_q_W_y',[L,J,Y]),tf.nn.softplus(tf.get_variable('scale_q_W_y',[L,J,Y]))+1e-35
...
def beta_kldiv(q_U,q_W_x,q_W_h,q_W_y,beta):
    return(beta*(tf.reduce_sum(q_U.kl_divergence(U)) + tf.reduce_sum(q_W_x.kl_divergence(W_x)) + tf.reduce_sum(q_W_h.kl_divergence(W_h)) + tf.reduce_sum(q_W_y.kl_divergence(W_y))))
...
kldiv = beta_kldiv(q_U,q_W_x,q_W_h,q_W_y,beta)
...
for c in range(4):
    ...
    for epoch in range(n_epochs):
        ...
        for t in range(n_batches):
            ...
            beta_ = (epoch*int(n_batches)+t)/(n_epochs*n_batches)
            _, ELBO_ = sess.run([(train_op,check_op), ELBO],feed_dict={langs:langs_batch,lens:lens_batch,inputs:inputs_batch,outputs:outputs_batch,beta:beta_})
            ELBOs.append(ELBO_)
            duration = time.time() - start_time
            print("Step: {:>3d}, beta={:.3f} Log ELBO: {:.3f} ({:.3f} sec)".format(epoch*int(n_batches)+t, beta_, ELBO_, duration))
